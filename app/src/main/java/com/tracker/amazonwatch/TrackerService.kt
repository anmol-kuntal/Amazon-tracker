    ...
    private suspend fun CoroutineScope.pollLoop() {
        ...
        while (isActive) {
            ...
            try {
                ...
                wakeLock?.let { if
cat > app/src/main/java/com/tracker/amazonwatch/TrackerService.kt <<'EOF'
package com.tracker.amazonwatch
...
class TrackerService : Service() {
    ...
    private suspend fun CoroutineScope.pollLoop() {
        ...
        while (isActive) {
            ...
            // The target interval covers the whole cycle (fetch + parse +
            // sleep), so subtract however long the fetch/parse just took -
            // otherwise a slow fetch silently stretches every cycle past
            // what's configured.
